...
def attendance_session():
    try:
        ...
        for studentid, db_encoding in recognition_data:
            ...
            if matches[0]:
                student_name = get_student_name(studentid)  # Get student name from DB
                if student_name:
                    return jsonify({
                        'recognized': True,
                        'studentid': studentid,
...
def get_student_name(studentid):
    """
    Fetches the student's name from the database using studentid.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT name FROM student WHERE studentid = %s", (studentid,))
...
def attendance1():
    try:
        ...
        for studentid, db_encoding in recognition_data:
            ...
            if matches[0]:
                is_new_record, student_name = check_and_insert_attendance(event_id, studentid)
                if student_name is None:
                    ...
                    return jsonify({
                        'recognized': False, 
                        'message': f'Attendance already recorded for {student_name}',
                        'name': student_name
                    })
                    
        # If no match found after checking all encodings
        return jsonify({'recognized': False, 'message': 'No match found'})
        
    except Exception as e:
        return jsonify({'recognized': False, 'message': f'Error in recognition: {str(e)}'}), 500
...
def check_and_insert_attendance(event_id, studentid):
    ...
    Retrieves student name and returns attendance status with student information.
    Returns tuple: (is_new_record: bool, student_name: str)
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        ...
        cursor.execute(
            "SELECT name FROM student WHERE studentid = %s",
            (studentid,)
        )
        student_result = cursor.fetchone()
        if not student_result:
            print(f"Student ID {studentid} not found in database")
            return False, None
        ...
        if result is None:
            ...
            conn.commit()
            print(f"Attendance recorded for {student_name} (ID: {studentid})")
            return True, student_name
        else:
            print(f"Duplicate attendance for {student_name} (ID: {studentid})")
...
def add_student():
    ...
    if not student_name or not student_id or not student_password:
    ...
    # Insert new student
    cursor.execute("INSERT INTO student (name, studentid, password) VALUES (%s, %s, %s)",
                   (student_name, student_id, student_password))
    conn.commit()
